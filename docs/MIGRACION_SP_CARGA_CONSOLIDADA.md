...
### Atomicidad — ENDURECIDA (V24)
**Resuelto.** La staging pasó a ser **`CREATE TEMPORARY TABLE`** (creada por Java): a diferencia de `CREATE/DROP TABLE`, las variantes `TEMPORARY` **no provocan commit implícito**, por lo que los writes del SP + el sync de clientes quedan dentro de la `@Transactional` de Spring y **revierten juntos** ante un fallo. Como las temporales no están en `information_schema`, el SP ya **no introspecciona**: Java pasa la lista de columnas y el `SET` ya armados (nombres sanitizados por `SqlSanitizer`). Migración `V24__harden_sp_import_temp.sql`. Validado contra `foh_cas_cst` real con `ROLLBACK` (upsert + update con COALESCE → las escrituras se revierten). Elimina además la dependencia de `group_concat_max_len`.
...
| **F3 — Validación / parallel-run** | **DB-level (reversible contra QAS):** `sp_import_upsert`, `sp_import_update` (COALESCE preserva NULL, not_found), colación heredada (join OK en `0900_ai_ci`; incorrecta → `ERROR 1267`), y atomicidad TEMPORARY (rollback revierte). **End-to-end en la VM (2026-06-11):** harness `update-complementary` corrido contra el servicio vivo con deploy `2f005ca`: motor **legacy** PASA y motor **`sp` PASA** (`updatedRows=3, notFoundRows=1`), con el path `[SP]` confirmado en `logs/app.log`. Falta e2e de `import-data` (upsert+sync) e `import-daily` (idealmente sobre subcartera desechable). | ✅ complementary e2e (legacy+sp); ⏳ upsert/daily e2e |
